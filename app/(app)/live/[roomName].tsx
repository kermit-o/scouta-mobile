...
export default function LiveRoomScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await joinStream(roomName as string);
        if (r.status === 200 && r.data && r.data.token) {
          setStatus("ok");
          setTitle(r.data.title || "");
          try {
            const ch = await fetch(API_BASE + "/live/" + roomName + "/chat?limit=50");
            const cd = await ch.json();
            if (cd.messages) setChat(cd.messages);
          } catch {}
          ...
          interval = setInterval(async function() {
            try {
              ...
              if (found) setViewers(found.viewer_count);
  ...
  async function doGift(g: GiftItem) {
    ...
    if (r.ok) setShowGifts(false);
    else Alert.alert("Error", r.detail || "Not enough coins");
  }

  function doEnd() {
    Alert.alert("End Stream?", "This will end the live stream for everyone.", [
